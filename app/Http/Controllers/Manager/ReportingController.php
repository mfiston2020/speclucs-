...
class ReportingController extends Controller
{
    public function index()
    {
        $product    =   DB::table('sold_products')->select(DB::raw('sum(quantity) as sold, product_id'))
                        ...
                        ->groupBy('product_id')->limit(5)->orderBy('sold','DESC')->get();

        $payment_method   =   DB::table('transactions')->select(DB::raw('sum(amount) as expenses, payment_method_id'))->where('company_id',Auth::user()->company_id)
                        ->groupBy('payment_method_id')->where('type','=','expense')->limit(5)
                        ->orderBy('expenses','desc')->get();

        $expenses   =   \App\Models\Transactions::where(['type'=>'expense'])->where('company_id',Auth::user()
                                                    ->company_id)->select('*')
                                                    ->orderBy('amount','DESC')
                                                    ->limit(5)->get();
