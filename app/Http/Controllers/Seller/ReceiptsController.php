...
class ReceiptsController extends Controller
{
    ...
    {
        $receipts   =   \App\Models\Receipt::where('company_id',Auth::user()->company_id)->get();
        return view('seller.recu.index',compact('receipts'));
    }

    public function add(){
        $provider   =   \App\Models\Supplier::where('company_id',Auth::user()->company_id)->get();
        return view('seller.recu.create',compact('provider'));
    }
    ...
    public function save(Request $request)
    {
        ...
        try {
            $receipt->save();
            return redirect()->route('seller.receipt.detail',Crypt::encrypt($receipt->id))->with('successMsg','Receipt Created Successfully');
        } catch (\Throwable $th) {
    ...
    public function detail($id){
        ...
        return view('seller.recu.detail',compact('receiptDetail','products'));
    }
    ...
    public function addProdcut($id)
    {
        ...
        // return view('seller.recu.addProduct',compact('products','id','categories','lens_types','chromatics','coatings','index'));
        return view('seller.recu.productAdd',compact('products','id','categories','lens_types','chromatics','coatings','index'));
    }
    ...
    public function saveProdcut(Request $request)
    {
        ...
        if ($existing_product==0) 
        {
            ...
            try {
                $products->save();
                return redirect()->route('seller.receipt.detail',Crypt::encrypt($request->receipt_id))->with('successMsg','Product has been successfully Added!');
            } catch (\Throwable $th) {
                //throw $th;
    ...
    public function editDetail($id)
    {
        ...
        return view('seller.recu.editProduct',compact('product'));
    }
    ...
    public function saveDetail(Request $request,$id)
    {
        ...
        try {
            $product->save();
            return redirect()->route('seller.receipt.detail',Crypt::encrypt($product->receipt_id))->with('successMsg','Product has been successfully Updated!');
        } catch (\Throwable $th) {
            //throw $th;
            return redirect()->back()->withInput()->with('errorMsg','Sorry Something Went Wrong! '.$th);
        }
    ...
    public function finalizeReceipt(Request $request,$id)
    {
        ...
        try {
            $receipt->save();
            return redirect()->route('seller.receipt.detail',Crypt::encrypt($id))->with('successMsg','Receipt has been successfully Finalized!');
        } catch (\Throwable $th) {
            //throw $th;
            return redirect()->back()->withInput()->with('errorMsg','Sorry Something Went Wrong! '.$th);
        }
    ...
    public function invoiceDetail($id)
    {
        ...
        return view('seller.receipt.detail',compact('invoice','products'));
    }
