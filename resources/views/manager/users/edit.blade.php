...
@section('title','Manager Dashboard - Edit User')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <form class="form-horizontal" action="{{route('manager.user.update',Crypt::encrypt($user->id))}}" method="POST">
                    @csrf
                    <div class="card-body">
                        {{-- ====== input error message ========== --}}
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <select class="select2 form-control custom-select" style="width: 100%; height:36px;"
                                    ...
                                    <option value="accountant" {{($user->permissions=='accountant')?'selected':''}}> Accountant </option>
                                    <option value="doctor" {{($user->permissions=='doctor')?'selected':''}}> Ophthalmologist</option>
                                    <option value="lab" {{($user->permissions=='lab')?'selected':''}}> Lab Technician </option>
                                    <option value="seller" {{($user->permissions=='seller')?'selected':''}}> Seller </option>
                                    <option value="store" {{($user->permissions=='store')?'selected':''}}> Store Keeper </option>
