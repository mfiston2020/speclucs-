                            ...
                            <table id="zero_config" class="table table-bordered nowrap">
                                ...
                                <tbody>
                                    @foreach ($pendingOrders as $key => $order)
                                                ...
                                                @if ($order->status != 'sold')
                                                    @if (userInfo()->role == 'store' || userInfo()->role == 'manager')
                                                        @if ($order->status == 'pending')
                                                            <a data-toggle="modal" data-target="#price-{{ $key }}"
                                                                class="btn btn-success btn-sm text-white">{{ __('manager/sales.add_price') }}</a>
                                                        @endif
                                                    @endif
                                                    ...
                                                    @if (userInfo()->role == 'seller' || userInfo()->role == 'manager')
                                                        @if ($order->status == 'approved')
                                                            <a data-toggle="modal"
                                                                data-target="#reaction-{{ $key }}"
                                                                class="btn btn-warning btn-sm text-white">{{ __('manager/sales.paid') }}</a>
                                                        @endif
                                                    @endif

                                                    @if (userInfo()->role == 'seller' || userInfo()->role == 'manager')
                                                        @if ($order->status == 'paid')
                                                            <a data-toggle="modal" data-target="#sell-{{ $key }}"
                                                                class="btn btn-primary btn-sm text-white">{{ __('manager/sales.sell') }}</a>
                                                        @endif
                                                    @endif
                                                    <a data-toggle="modal" data-target="#cancel-{{ $key }}"
                                                        class="text-danger ml-4">{{ __('manager/sales.cancel_order') }}</a>
                                                @else
                                        ...
                                        {{-- setting price modal --}}
                                        <div id="price-{{ $key }}" class="modal fade" tabindex="-1"
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <h5 class="text-secondary">{{ __('manager/sales.order_detail') }}
                                                        </h5>
                                                        <br>
                                                        {{-- <div class="row"> --}}
                                                        ...
                                                        <span>
                                                            ...
                                                            {{ \App\Models\PhotoCoating::where('id', $order->coating_id)->pluck('name')->first() }}
                                                            - {{ $order->sphere_r == null ? 'left' : 'right' }}
                                                        </span>
                                                        {{-- </div> --}}
                                                        <br>
                                                        <br>
                                                        <div class="row">
                                                            <div class="row col-md-3">
                                                                ...
                                                                <span
                                                                    class="col-12">{{ $order->sphere_r == null ? format_values($order->sphere_l) : format_values($order->sphere_r) }}</span>
                                                            </div>

                                                            <div class="row col-md-3">
                                                                <strong class="col-12">Cylinder: </strong>
                                                                <span
                                                                    class="col-12">{{ $order->cylinder_r == null ? format_values($order->cylinder_l) : format_values($order->cylinder_r) }}</span>
                                                            </div>

                                                            <div class="row col-md-3">
                                                                <strong class="col-12">Axis: </strong>
                                                                <span
                                                                    class="col-12">{{ $order->axis_r == null ? format_values($order->axis_l) : format_values($order->axis_r) }}</span>
                                                            </div>

                                                            <div class="row col-md-3">
                                                                <strong class="col-12">Addition: </strong>
                                                                <span
                                                                    class="col-12">{{ $order->addition_r == null ? format_values($order->addition_l) : format_values($order->addition_r) }}</span>
                                                            </div>
                                                        ...
                                                        @if ($order->status != 'sold')
                                                            ...
                                                            <form action="{{ route('manager.pending.order.price') }}"
                                                                method="post" id="setPriceForm-{{ $key }}">
                                                                @csrf
                                                    ...
                                                    <div class="modal-footer">
                                                        @if ($order->status != 'sold')
                                                            <button class="btn btn-info waves-effect"
                                                                onclick="document.getElementById('setPriceForm-{{ $key }}').submit()">{{ __('manager/sales.add_price') }}</button>
                                                        @endif
                                        ...
                                        <div id="reaction-{{ $key }}" class="modal fade" tabindex="-1"
                                            role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <form action="{{ route('manager.sell.na.product') }}"
                                                            method="post" id="reactionForm-{{ $key }}">
                                                            @csrf
                                                            <input type="hidden" name="thisName"
                                                                value="{{ Crypt::encrypt($order->id) }}">
                                                        </form>
                                                    ...
                                                    <div class="modal-footer">
                                                        <button
                                                            onclick="document.getElementById('reactionForm-{{ $key }}').submit()"
                                        ...
                                        {{-- client sell modal --}}
                                        <div id="sell-{{ $key }}" class="modal fade" tabindex="-1"
                                            role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                    ...
                                                    <div class="modal-footer">
                                                        <button
                                                            onclick="document.getElementById('sellForm-{{ $key }}').submit()"
                                                            class="btn btn-info waves-effect">{{ __('navigation.yes') }}</button>
                                                        <button type="button" class="btn btn-danger waves-effect"
                                                            data-dismiss="modal">{{ __('navigation.no') }}</button>
                                                    </div>
                                        ...
                                        {{-- cancel modal --}}
                                        <div id="cancel-{{ $key }}" class="modal fade" tabindex="-1"
                                            role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                    ...
                                                    <div class="modal-footer">
                                                        <button
                                                            onclick="document.getElementById('cancelForm-{{ $key }}').submit()"
                                                            class="btn btn-info waves-effect">{{ __('navigation.yes') }}</button>
                                                        <button type="button" class="btn btn-danger waves-effect"
                                                            data-dismiss="modal">{{ __('navigation.no') }}</button>
                                                    </div>
