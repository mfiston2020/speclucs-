            ...
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        ...
                        <a href="{{route('manager.payment.add')}}" type="button" class="btn waves-effect waves-light btn-rounded btn-outline-primary" style="align-items: right;">
                            ...
                            <tbody>
                                @foreach ($payments as $key=> $payment)
                                    <tr>
                                        ...
                                        <td><a href="{{route('manager.receipt.detail',Crypt::encrypt($payment->receipt_id))}};">
                                        ...
                                        <td>{{\App\Models\PaymentMethod::where(['id'=>$payment->payment_method_id])->pluck('name')->first()}}</td>
                                        <td>{{\App\Models\Receipt::where(['id'=>$payment->receipt_id])->where('company_id',Auth::user()->company_id)->pluck('total_cost')->first()}}</td>
                                        <td>{{$payment->due}}</td>
                                        {{-- <td>
                                            <a href="{{route('manager.edit.receipt',Crypt::encrypt($payment->id))}}" style="color: rgb(0, 38, 255)">Edit</a>
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <a href="{{route('manager.sales.remove.product',Crypt::encrypt($payment->id))}}"
